...
- [`output/`](output): The output samples
	- [`samples-summary.json`](output/samples-summary.json): Summary of OS identifying information (generated by `playbooks/parse-samples.yml`)
	- [`ostest.samples.md`](output/ostest.samples.md): Manually collected samples
- [`playbooks/gather-all.yml`](playbooks/gather-all.yml): A playbook to gather all available facts with the `ansible.builtin.setup` & `ansible.builtin.package_facts` modules
- [`playbooks/gather-subset.yml`](playbooks/gather-subset.yml): A playbook to gather available facts by individual subset, to facilitate collection of only the relevant facts
